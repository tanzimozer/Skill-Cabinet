...
12. **Headless VM auth (Lima/SSH)** — `claude auth login` opens a browser on the *host machine*, not where you're SSHed from. The VM and host have separate auth states. **Recommended approach: OAuth token transfer.**

    **Step 1 — On the host machine (Mac, with browser access), authenticate and generate a token:**
    ```bash
    # Authenticate if not already logged in
    claude auth login
    # Generate a long-lived token (valid 1 year, requires Max/Pro subscription)
    claude setup-token
    # Outputs: sk-ant-oat01-...
    ```

    **Step 2 — In the headless VM, set the token as an environment variable:**
    ```bash
    ...
    # Verify
    claude auth status
    # Should show: loggedIn: true, authMethod: oauth_token, apiProvider: firstParty
    ```

    **Key points:**
    - `apiProvider: firstParty` = billing to your subscription (Max/Pro), not API wallet
    - Token is user-specific — each VM user needs it in their own `~/.bashrc`
    - Lima VMs don't share clipboard with macOS — use `limactl shell <vm> -- <command>` to pipe or type manually
    - Token lasts 1 year; regenerate with `claude setup-token` when it expires
