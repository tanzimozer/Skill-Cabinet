...
## Replicating Live Site Designs

When the user wants to **clone an existing site** (e.g., "build a site like blog.ultrahuman.com"):

1. See `references/design-analysis-workflow.md` for the full pattern
2. Screenshot the target site (Playwright if browser tools time out)
3. Extract design tokens (colors, typography, spacing, layout) to structured JSON
4. Optionally create a Trello board breakdown for complex builds
5. Build in HTML/CSS/JS (not in visual builders — they lack APIs)
6. Deliver via Google Drive or GitHub with deployment guide

**Key insight:** Visual site builders (Webflow, Squarespace, Framer) have no design automation APIs. 
When users ask to "build in Webflow," pivot to code-based builds that can be deployed anywhere.
...
**Robinhood Gold** (no template — see `references/robinhood-gold-design-system.md`): Pure `#000000` background, primary gold `#A89462` (H=43°, L=52% — darker than typical fintech gold), used *very* sparingly. Warm cream text `#E0DCD8`. **No red, purple, or blue** in the palette — strictly black + gold + green (positive) + cream. Deep pixel-verified June 2026 — previous `#C9A84C` value was wrong (too bright, wrong hue). Used as a design-language source (not a brand clone) — apply when user asks for "Robinhood-inspired" or "fintech dark gold" aesthetic.
...
See `references/timbr-ui-mockup.md` for the full Timbr iOS workout logger build — screens, corrections, versioning, and drop workflow.
